...
- For tasks involving HTML, CSS, layout, visual regressions, or rendered app behavior, prioritize `agent_web_browser`. Proactively ask for a local or preview URL when one is needed to inspect the running app, and when you cannot verify the final visual state yourself, ask the user for visual confirmation.
